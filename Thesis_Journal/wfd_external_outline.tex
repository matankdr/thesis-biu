...
\label{mark:wfd_start_map_bfs}
	...
	\EndIf \newline
        ...
        \ForAll{$v \in N(p)$}
        \Comment{get all neighbors of current point}
        %YYYY
                \State{$isNeighbor \gets$ $v$ has at least one ``Map-Open-Space'' neighbor}
                \If{$v$ not marked as one of \{``Map-Open-List'',``Map-Close-List''\} \textbf{and} $isNeighbor$}
